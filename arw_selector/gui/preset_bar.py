...
class PresetBar(QWidget):
    ...
    def _on_selected(self, index: int) -> None:
        ...
        # _apply can fail too - a preset is YAML you can edit by hand, so
        # one odd value blows up in deserialisation. Left outside the try,
        # that exception goes straight up and, above all, _loading stays
        # True so every later slider move silently does nothing.
        self._loading = True
        try:
            self._apply(data)
        except Exception as exc:  # noqa: BLE001 - no value may stop the window
            QMessageBox.warning(
                self, tr("Preset"), tr("Could not load:\n{error}").format(error=exc))
            return
        finally:
